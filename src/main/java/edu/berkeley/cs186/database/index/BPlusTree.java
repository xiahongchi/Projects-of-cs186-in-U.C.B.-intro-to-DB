...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> putRes = root.put(key, rid);
        if(putRes.isPresent()){
            long leftPageNum = root.getPage().getPageNum();
            long rightPageNum = putRes.get().getSecond();
            DataBox split_key = putRes.get().getFirst();

            List<DataBox> newKeys = new ArrayList<>();
            List<Long> newChildren = new ArrayList<>();

            newKeys.add(split_key);
            newChildren.add(leftPageNum);
            newChildren.add(rightPageNum);

            updateRoot(new InnerNode(this.metadata, this.bufferManager, newKeys, newChildren, this.lockContext));
        }
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        byte[] BytesRepresent = root.toBytes();
        if(BytesRepresent[0] == 1 && BytesRepresent[9] == 0 && BytesRepresent[10] == 0
            && BytesRepresent[11] == 0 && BytesRepresent[12] == 0){
            while (data.hasNext()){
                Optional<Pair<DataBox, Long>> loadRes = root.bulkLoad(data, fillFactor);

                if(loadRes.isPresent()){
                    long leftPageNum = root.getPage().getPageNum();
                    long rightPageNum = loadRes.get().getSecond();
                    DataBox split_key = loadRes.get().getFirst();
            ...
            throw new BPlusTreeException("load to a non-empty tree");
        }
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public BPlusTreeIterator(DataBox key){
            ...
            curr = root.get(key);
            int i;
            for(i = 0; i < curr.getKeys().size(); i++){
                if(curr.getKeys().get(i).compareTo(key) >= 0){
                    index = i;
                    break;
                }
            }
            if(i == curr.getKeys().size()){
                index = curr.getKeys().size();
                if(curr.getRightSibling().isPresent()){
                    index = 0;
                    curr = LeafNode.fromBytes(BPlusTree.this.getMetadata(), BPlusTree.this.bufferManager,
                            BPlusTree.this.lockContext, curr.getRightSibling().get().getPage().getPageNum());
                }
            }
        }
        ...
        public boolean hasNext() {
            // TODO(proj2): implement
            return curr.getRightSibling().isPresent() || index < curr.getKeys().size();
        }
        ...
        public RecordId next() {
            ...
            RecordId rs = curr.getRids().get(index);
            index++;
            if(index == curr.getKeys().size()){
                if(curr.getRightSibling().isPresent()) {
                    index = 0;
                    curr = LeafNode.fromBytes(BPlusTree.this.getMetadata(), BPlusTree.this.bufferManager,
                            BPlusTree.this.lockContext, curr.getRightSibling().get().getPage().getPageNum());
