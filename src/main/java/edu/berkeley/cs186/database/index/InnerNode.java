...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        ...
        for(int i = 0; i < this.keys.size(); ++i){
            if(key.compareTo(this.keys.get(i)) < 0){
                return BPlusNode.fromBytes(this.metadata,this.bufferManager,this.treeContext,this.children.get(i)).get(key);
            }
            else if(i == this.keys.size() - 1){
                return BPlusNode.fromBytes(this.metadata,this.bufferManager,this.treeContext,this.children.get(i+1)).get(key);
            }
        }
        return null;
    }
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> putRes = Optional.empty();

        for(int i = 0; i < this.keys.size(); ++i){
            if(key.compareTo(this.keys.get(i)) < 0){
                putRes = BPlusNode.fromBytes(this.metadata,this.bufferManager,this.treeContext,this.children.get(i)).put(key, rid);
                break;
            }
            else if(i == this.keys.size() - 1){
                putRes = BPlusNode.fromBytes(this.metadata,this.bufferManager,this.treeContext,this.children.get(i+1)).put(key, rid);
                break;
            }
        }

        if(putRes.isPresent()){
            Pair<DataBox, Long> putPair = putRes.get();
            DataBox split_key = putPair.getFirst();
            Long right_node_page_num = putPair.getSecond();
            this.keys.add(split_key);
            Collections.sort(this.keys);
            for(int i = 0; i < this.keys.size(); i++){
                if(split_key.equals(this.keys.get(i))){
                    this.children.add(i+1, right_node_page_num);
                    break;
                }
            }
            if(this.keys.size() == this.metadata.getOrder() * 2 + 1){
                int originSize = this.keys.size();
                List<DataBox> newKeys = new ArrayList<>();
                List<Long> newChildren = new ArrayList<>();

                for(int i = this.metadata.getOrder() + 1; i < originSize; ++i){
                    newKeys.add(this.keys.get(i));
                    newChildren.add(this.children.get(i));
                }
                newChildren.add(this.children.get(originSize));
                ...
                InnerNode newInner = new InnerNode(this.metadata, this.bufferManager, newKeys,
                        newChildren, this.treeContext);

                DataBox new_split_key = this.keys.get(this.metadata.getOrder());
                this.keys.remove(this.keys.size() - 1);

                sync();
                return Optional.of(new Pair<>(new_split_key, newInner.getPage().getPageNum()));
            }
            else{
                sync();
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
            float fillFactor) {
        // TODO(proj2): implement
        Optional<Pair<DataBox, Long>> loadRes = BPlusNode.fromBytes(metadata, bufferManager, treeContext,
                this.children.get(this.children.size() - 1)).bulkLoad(data, fillFactor);
        if(loadRes.isPresent()){
            Pair<DataBox, Long> loadPair = loadRes.get();
            DataBox split_key = loadPair.getFirst();
            Long right_node_page_num = loadPair.getSecond();
            this.keys.add(split_key);
    ...
    public void remove(DataBox key) {
        ...
        for(int i = 0; i < this.keys.size(); i++){
            if(key.compareTo(this.keys.get(i)) < 0){
                BPlusNode.fromBytes(this.metadata, this.bufferManager, this.treeContext, this.children.get(i)).remove(key);
                return;
            }
        }
        BPlusNode.fromBytes(this.metadata, this.bufferManager, this.treeContext, this.children.get(this.keys.size())).remove(key);
        return;
    }
