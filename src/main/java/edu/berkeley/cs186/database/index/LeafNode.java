...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        int order = metadata.getOrder(), size = keys.size();
        ...
        if(size == 2 * order){
            ...
            for(int i = order; i < originSize; i++){
                this.keys.remove(this.keys.size() - 1);
                this.rids.remove(this.rids.size() - 1);
            }
            LeafNode newLeaf = new LeafNode(this.metadata, this.bufferManager, newKeys,
                    newRids, this.rightSibling, this.treeContext);

            this.rightSibling = Optional.of(newLeaf.page.getPageNum());
            sync();
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        while(data.hasNext()){
            ...
            Pair<DataBox, RecordId> NextElem = data.next();
            DataBox NextKey = NextElem.getFirst();
            RecordId NextRecord = NextElem.getSecond();

            if(( (((float) this.keys.size()) / (2 * (float) this.metadata.getOrder()))) >= fillFactor){
                List<DataBox> newKeys = new ArrayList<>();
                List<RecordId> newRids = new ArrayList<>();

                newKeys.add(NextKey);
                newRids.add(NextRecord);
                ...
                sync();
                return Optional.of(new Pair<>(NextKey, newLeaf.getPage().getPageNum()));
            }
            else{
                this.keys.add(NextKey);
                this.rids.add(NextRecord);
